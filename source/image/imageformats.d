...
PNG_Header read_png_header(Reader stream)
{
	...
	PNG_Header header = {
	width:
		bigEndianToNative!int(tmp[16 .. 20]), height : bigEndianToNative!int(tmp[20 .. 24]),
	bit_depth : tmp[24], color_type : tmp[25], compression_method : tmp[26],
	filter_method : tmp[27], interlace_method : tmp[28],
	};
	return header;
}
...
IFImage read_png(Reader stream, long req_chans = 0)
{
	...
	if (!(hdr.color_type == PNG_ColorType.Y || hdr.color_type == PNG_ColorType.RGB
	...
	PNG_Decoder dc = {
	stream:
		stream, src_indexed : (hdr.color_type == PNG_ColorType.Idx), src_chans
			: channels(cast(PNG_ColorType) hdr.color_type), ilace
			: hdr.interlace_method, w : hdr.width, h : hdr.height,
	};
...
immutable ubyte[8] png_file_header = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a];
...
ubyte[] decode_png(ref PNG_Decoder dc)
{
	...
	while (stage != Stage.IEND_parsed)
	{
		...
		{ // chunk type
		case "IDAT":
			if (!(stage == Stage.IHDR_parsed || (stage == Stage.PLTE_parsed && dc.src_indexed)))
...
ubyte[] read_IDAT_stream(ref PNG_Decoder dc, int len)
{
	...
	else
	{
		...
		immutable size_t[7] redw = [(dc.w + 7) / 8, (dc.w + 3) / 8,
			(dc.w + 3) / 4, (dc.w + 1) / 4, (dc.w + 1) / 2, (dc.w + 0) / 2, (dc.w + 0) / 1,];
		immutable size_t[7] redh = [(dc.h + 7) / 8, (dc.h + 7) / 8,
			(dc.h + 3) / 8, (dc.h + 3) / 4, (dc.h + 1) / 4, (dc.h + 1) / 2, (dc.h + 0) / 2,];
		...
		foreach (pass; 0 .. 7)
		{
			...
			foreach (j; 0 .. redh[pass])
			{
				...
				for (size_t i, redi; i < redw[pass]; ++i, redi += dc.tgt_chans)
				{
					...
					result[tgt .. tgt + dc.tgt_chans] = redlinebuf[redi .. redi + dc.tgt_chans];
				}
...
immutable A7_Catapult[7] a7_catapults = [
	&a7_red1_to_dst, &a7_red2_to_dst, &a7_red3_to_dst, &a7_red4_to_dst,
	&a7_red5_to_dst, &a7_red6_to_dst, &a7_red7_to_dst,
];
...
void write_png(Writer stream, long w, long h, in ubyte[] data, long tgt_chans = 0)
{
	...
	PNG_Encoder ec = {
	stream:
		stream, w : cast(size_t) w, h : cast(size_t) h, src_chans : src_chans,
...
void write_png(ref PNG_Encoder ec)
{
	...
	static immutable ubyte[12] iend = [0, 0, 0, 0, 'I', 'E', 'N', 'D', 0xae, 0x42, 0x60, 0x82];
...
void write_IDATs(ref PNG_Encoder ec)
{
	...
	foreach (j; 0 .. ec.h)
	{
		...
		foreach (i; filter_step + 1 .. cline.length)
			filtered_line[i] = cast(ubyte)(cline[i] - paeth(cline[i - filter_step],
...
void write_to_IDAT_stream(ref PNG_Encoder ec, in void[] _compressed)
{
	...
	while (compressed.length)
	{
		...
		ec.data_buf[ec.writelen .. ec.writelen + writenow_len] = compressed[0 .. writenow_len];
...
TGA_Header read_tga_header(Reader stream)
{
	...
	TGA_Header header = {
	id_length:
		tmp[0], palette_type : tmp[1], data_type : tmp[2], palette_start
			: littleEndianToNative!ushort(tmp[3 .. 5]), palette_length : littleEndianToNative!ushort(tmp[5 .. 7]),
	palette_bits : tmp[7], x_origin : littleEndianToNative!ushort(tmp[8 .. 10]),
	y_origin : littleEndianToNative!ushort(tmp[10 .. 12]), width
			: littleEndianToNative!ushort(tmp[12 .. 14]), height
			: littleEndianToNative!ushort(tmp[14 .. 16]), bits_pp : tmp[16], flags : tmp[17],
	};
	return header;
}
...
IFImage read_tga(Reader stream, long req_chans = 0)
{
	...
	TGA_Decoder dc = {
	stream:
		stream, w : hdr.width, h : hdr.height, origin_at_top
			: cast(bool)(hdr.flags & 0x20), bytes_pp : hdr.bits_pp / 8, rle : rle,
	...
	IFImage result = {
	w:
		dc.w, h : dc.h, c : cast(ColFmt) dc.tgt_chans, pixels : decode_tga(dc),
	};
	return result;
}
...
void write_tga(Writer stream, long w, long h, in ubyte[] data, long tgt_chans = 0)
{
	...
	TGA_Encoder ec = {
	stream:
		stream, w : cast(ushort) w, h : cast(ushort) h, src_chans
			: cast(int) src_chans, tgt_chans : cast(int)((tgt_chans) ? tgt_chans
					: src_chans), rle : true, data : data,
	};
...
BMP_Header read_bmp_header(Reader stream)
{
	...
	if (1 <= dib_version)
	{
		DibV1 v1 = {
		bits_pp:
			cast(size_t) littleEndianToNative!ushort(dib_header[10 .. 12]),
	...
	if (2 <= dib_version)
	{
		DibV2 v2 = {
		red_mask:
			littleEndianToNative!uint(dib_header[36 .. 40]), green_mask
				: littleEndianToNative!uint(dib_header[40 .. 44]),
	...
	if (4 <= dib_version)
	{
		DibV4 v4 = {
		color_space_type:
			littleEndianToNative!uint(dib_header[52 .. 56]), color_space_endpoints : dib_header[56 .. 92], gamma_red
				: littleEndianToNative!uint(dib_header[92 .. 96]), gamma_green
				: littleEndianToNative!uint(dib_header[96 .. 100]),
		gamma_blue : littleEndianToNative!uint(dib_header[100 .. 104]), intent
				: littleEndianToNative!uint(dib_header[104 .. 108]),
		};
		dib_v4 = v4;
	}
	...
	if (5 <= dib_version)
	{
		DibV5 v5 = {
		icc_profile_data:
			littleEndianToNative!uint(dib_header[108 .. 112]), icc_profile_size
				: littleEndianToNative!uint(dib_header[112 .. 116]),
		};
		dib_v5 = v5;
	}
	...
	BMP_Header header = {
	file_size:
		cast(size_t) littleEndianToNative!uint(tmp[2 .. 6]), pixel_data_offset
			: cast(size_t) littleEndianToNative!uint(tmp[10 .. 14]), width
			: littleEndianToNative!int(dib_header[0 .. 4]),
	height : littleEndianToNative!int(dib_header[4 .. 8]), planes
			: littleEndianToNative!ushort(dib_header[8 .. 10]), dib_version
			: dib_version, dib_v1 : dib_v1, dib_v2 : dib_v2,
	dib_v3_alpha_mask : dib_v3_alpha_mask, dib_v4 : dib_v4, dib_v5 : dib_v5,
	};
	return header;
}
...
IFImage read_bmp(Reader stream, long req_chans = 0)
{
	...
	if (hdr.pixel_data_offset < (14 + hdr.dib_size) || hdr.pixel_data_offset > 0xffffff /* arbitrary */ )
	{
	...
	if (1 <= hdr.dib_version)
	{
		...
		if (hdr.dib_v1.bits_pp <= 8 && (hdr.dib_v1.palette_length == 0
	...
	size_t mask_to_idx(uint mask)
	{
		...
		case 0x0000_00ff:
			return 0;
		default:
			throw new ImageIOException("unsupported mask");
		}
	}
	...
	immutable tgt_chans = (0 < req_chans) ? req_chans : (alpha_masked) ? _ColFmt.RGBA : _ColFmt.RGB;
	...
	foreach (_; 0 .. abs(hdr.height))
	{
		...
		else
		{
			for (size_t si, di; si < src_line.length; si += bytes_pp, di += 4)
			{
				...
				bgra_line_buf[di + 3] = (alpha_masked) ? src_line[si + alphai] : 255;
			}
	...
	IFImage ret = {
	w:
		hdr.width, h : abs(hdr.height), c : cast(ColFmt) tgt_chans, pixels : result,
	};
	return ret;
}
...
void read_bmp_info(Reader stream, out long w, out long h, out long chans)
{
	...
	w = abs(hdr.width);
	h = abs(hdr.height);
	chans = (hdr.dib_version >= 3 && hdr.dib_v3_alpha_mask != 0) ? ColFmt.RGBA : ColFmt.RGB;
}
...
IFImage read_jpeg(Reader stream, long req_chans = 0)
{
	...
	IFImage result = {
	w:
		dc.width, h : dc.height, c : cast(ColFmt) dc.tgt_chans, pixels : decode_jpeg(dc),
	};
	return result;
}
...
void read_markers(ref JPEG_Decoder dc)
{
	...
	while (!has_next_scan_header && !dc.eoi_reached)
	{
		...
		case COM:
			...
			break;
		default:
			throw new ImageIOException("invalid / unsupported marker");
		}
	}
}
...
void read_frame_header(ref JPEG_Decoder dc)
{
	...
	if (precision != 8 || (dc.num_comps != 1 && dc.num_comps != 3) || len != 8 + dc.num_comps * 3)
		throw new ImageIOException("invalid / not supported");
	...
	foreach (i; 0 .. dc.num_comps)
	{
		...
		if ((dc.correct_comp_ids && ci != i + 1) || (!dc.correct_comp_ids && ci != i))
		...
		if (comp.sfy < 1 || 4 < comp.sfy || comp.sfx < 1 || 4 < comp.sfx || 3 < comp.qtable)
			throw new ImageIOException("invalid / not supported");
	...
	foreach (i; 0 .. dc.num_comps)
	{
		...
		debug (DebugJPEG)
			writefln("%d comp %d sfx/sfy: %d/%d", i, dc.comps[i].id,
					dc.comps[i].sfx, dc.comps[i].sfy);
	}
...
void read_scan_header(ref JPEG_Decoder dc)
{
	...
	foreach (i; 0 .. num_scan_comps)
	{
		...
		while (ci < dc.num_comps && dc.comps[ci].id != comp_id)
			++ci;
		if (dc.num_comps <= ci)
			throw new ImageIOException("invalid component id");
...
immutable ubyte[64] dezigzag = [0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18,
...
ubyte[] reconstruct(in ref JPEG_Decoder dc)
{
	...
	case 34, 33:
		...
		foreach (j; 0 .. dc.height)
		{
			...
			{
				result[di .. di + 3] = ycbcr_to_rgb(dc.comps[0].data[si + i],
						dc.comps[1].data[si + i], dc.comps[2].data[si + i],);
				if (dc.tgt_chans == 4)
					result[di + 3] = 255;
				di += dc.tgt_chans;
			}
...
ubyte[] upsample_rgb(in ref JPEG_Decoder dc, ubyte[] result)
{
	...
	foreach (j; 0 .. dc.height)
	{
		...
		foreach (i; 0 .. dc.width)
		{
			result[di .. di + 3] = ycbcr_to_rgb(dc.comps[0].data[si0 + cast(size_t) floor(i * si0xratio)],
...
pure void STBI__IDCT_1D(ref int t0, ref int t1, ref int t2, ref int t3, ref int x0,
		ref int x1, ref int x2, ref int x3, int s0, int s1, int s2, int s3,
...
pure void stbi__idct_block(ubyte* dst, long dst_stride, in ref short[64] data)
{
	...
	for (i = 0; i < 8; ++i, ++d, ++v)
	{
		...
		else
		{
			...
			STBI__IDCT_1D(t0, t1, t2, t3, x0, x1, x2, x3, d[0], d[8], d[16],
					d[24], d[32], d[40], d[48], d[56]);
	...
	for (i = 0, v = val.ptr; i < 8; ++i, v += 8, o += dst_stride)
	{
		...
		int t0, t1, t2, t3, x0, x1, x2, x3;
		STBI__IDCT_1D(t0, t1, t2, t3, x0, x1, x2, x3, v[0], v[1], v[2], v[3],
				v[4], v[5], v[6], v[7]);
...
void read_jpeg_info(Reader stream, out long w, out long h, out long chans)
{
	...
	while (true)
	{
		...
		case SKIP:
			...
			break;
		default:
			throw new ImageIOException("unsupported marker");
		}
	}
	assert(0);
}
...
private:
	...
	void file_seek(ptrdiff_t offset, int origin)
	{
...
private:
	...
	ubyte[] buffer;
	void mem_rawWrite(in ubyte[] block)
	{
		this.buffer ~= block;
	}
