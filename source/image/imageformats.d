...
void write_image(in char[] file, long w, long h, in ubyte[] data, long req_chans = 0)
{
	...
	{
	case "png":
		write_image = &write_png;
		break;
	case "tga":
		write_image = &write_tga;
		break;
	default:
		throw new ImageIOException("unknown image extension/type");
	}
...
void read_image_info(in char[] file, out long w, out long h, out long chans)
{
	...
	{
	case "png":
		read_image_info = &read_png_info;
		break;
	case "tga":
		read_image_info = &read_tga_info;
		break;
	case "bmp":
		read_image_info = &read_bmp_info;
		break;
	case "jpg":
		read_image_info = &read_jpeg_info;
		break;
	case "jpeg":
		read_image_info = &read_jpeg_info;
		break;
...
{
	@safe pure const this(string msg, string file = __FILE__, size_t line = __LINE__)
	{
		super(msg, file, line);
	}
}
...
PNG_Header read_png_header(Reader stream)
{
	...
	reverse(crc[]);
	if (tmp[0 .. 8] != png_file_header[0 .. $] || tmp[8 .. 16] != [0x0, 0x0, 0x0,
			0xd, 'I', 'H', 'D', 'R'] || crc != tmp[29 .. 33])
		throw new ImageIOException("corrupt header");

	PNG_Header header = {
	width:
		bigEndianToNative!int(tmp[16 .. 20]),
	height : bigEndianToNative!int(tmp[20 .. 24]), bit_depth : tmp[24],
	color_type : tmp[25], compression_method : tmp[26], filter_method : tmp[27],
	...
	IFImage read_png(Reader stream, long req_chans = 0)
	{
		...
		if (!(hdr.color_type == PNG_ColorType.Y
				...
				|| hdr.color_type == PNG_ColorType.YA || hdr.color_type == PNG_ColorType.RGBA))
		...
		PNG_Decoder dc = {
		stream:
			stream, src_indexed : (hdr.color_type == PNG_ColorType.Idx),
		src_chans : channels(cast(PNG_ColorType) hdr.color_type),
		ilace : hdr.interlace_method, w : hdr.width, h : hdr.height,};
			dc.tgt_chans = (req_chans == 0) ? dc.src_chans : cast(int) req_chans;

			IFImage result = {
			w:
				dc.w, h : dc.h, c : cast(ColFmt) dc.tgt_chans, pixels : decode_png(dc)};
				return result;
			}

			public void write_png(in char[] file, long w, long h, in ubyte[] data,
				long tgt_chans = 0)
			{
				scope writer = new Writer(file);
			...
			immutable ubyte[8] png_file_header = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a,
				0x1a, 0x0a];
			...
			PNG_ColorType color_type(long channels) pure nothrow
			{
				...
				{
				case 1:
					return PNG_ColorType.Y;
				case 2:
					return PNG_ColorType.YA;
				case 3:
					return PNG_ColorType.RGB;
				case 4:
					return PNG_ColorType.RGBA;
				default:
					assert(0);
				}
			}
			...
			ubyte[] decode_png(ref PNG_Decoder dc)
			{
				...
				while (stage != Stage.IEND_parsed)
				{
					...
					case "IEND":
						...
						if (len != 0 || dc.chunkmeta[0 .. 4] != [0xae, 0x42, 0x60,
								0x82])
							throw new ImageIOException("corrupt chunk");
			...
			ubyte[] read_IDAT_stream(ref PNG_Decoder dc, int len)
			{
				...
				else
				{
					...
					immutable size_t[7] redw = [(dc.w + 7) / 8, (dc.w + 3) / 8,
						(dc.w + 3) / 4, (dc.w + 1) / 4, (dc.w + 1) / 2,
						(dc.w + 0) / 2, (dc.w + 0) / 1,];
					immutable size_t[7] redh = [(dc.h + 7) / 8, (dc.h + 7) / 8,
						(dc.h + 3) / 8, (dc.h + 3) / 4, (dc.h + 1) / 4,
						(dc.h + 1) / 2, (dc.h + 0) / 2,];
					...
					foreach (pass; 0 .. 7)
					{
						...
						foreach (j; 0 .. redh[pass])
						{
							...
							for (size_t i, redi; i < redw[pass]; ++i, redi += dc.tgt_chans)
							{
								...
								result[tgt .. tgt + dc.tgt_chans] = redlinebuf[redi
									.. redi + dc.tgt_chans];
							}
			...
			immutable A7_Catapult[7] a7_catapults = [
				&a7_red1_to_dst, &a7_red2_to_dst, &a7_red3_to_dst,
				&a7_red4_to_dst, &a7_red5_to_dst, &a7_red6_to_dst, &a7_red7_to_dst,
			];
			...
			void uncompress_line(ref PNG_Decoder dc, ref int length, ref bool metaready,
				ubyte[] dst)
			{
			...
			void recon(ubyte[] cline, in ubyte[] pline, ubyte ftype, int fstep) pure
			{
				...
				case Average:
					foreach (k; 0 .. fstep)
						cline[k] += pline[k] / 2;
					foreach (k; fstep .. cline.length)
						cline[k] += cast(ubyte)((cast(uint) cline[k - fstep] + cast(uint) pline[k]) / 2);
					break;
				case Paeth:
					foreach (i; 0 .. fstep)
			...
			void write_png(Writer stream, long w, long h, in ubyte[] data, long tgt_chans = 0)
			{
				...
				PNG_Encoder ec = {
				stream:
					stream, w : cast(size_t) w, h : cast(size_t) h,
				src_chans : src_chans,
				tgt_chans : tgt_chans ? cast(uint) tgt_chans : src_chans, data : data,};
				...
				void write_IDATs(ref PNG_Encoder ec)
				{
					...
					foreach (j; 0 .. ec.h)
					{
						...
						foreach (i; 1 .. filter_step + 1)
							filtered_line[i] = cast(ubyte)(cline[i] - paeth(0, pline[i],
								0));
						foreach (i; filter_step + 1 .. cline.length)
							filtered_line[i] = cast(ubyte)(
								cline[i] - paeth(cline[i - filter_step],
								pline[i], pline[i - filter_step]));
				...
				void write_to_IDAT_stream(ref PNG_Encoder ec, in void[] _compressed)
				{
					...
					while (compressed.length)
					{
						...
						ec.data_buf[ec.writelen .. ec.writelen + writenow_len] = compressed[0
							.. writenow_len];
				...
				TGA_Header read_tga_header(Reader stream)
				{
					...
					TGA_Header header = {
					id_length:
						tmp[0], palette_type : tmp[1], data_type : tmp[2],
					...
					IFImage read_tga(Reader stream, long req_chans = 0)
					{
						...
						case Gray_RLE:
							...
							break;
						default:
							throw new ImageIOException("data type not supported");
						}
						...
						TGA_Decoder dc = {
						stream:
							stream, w : hdr.width, h : hdr.height,
						origin_at_top : cast(bool)(hdr.flags & 0x20),
						bytes_pp : hdr.bits_pp / 8, rle : rle,
						tgt_chans : (req_chans == 0) ? src_chans : cast(int) req_chans,};

							switch (dc.bytes_pp)
							{
							case 1:
								dc.src_fmt = _ColFmt.Y;
								break;
							case 2:
								dc.src_fmt = _ColFmt.YA;
								break;
							case 3:
								dc.src_fmt = _ColFmt.BGR;
								break;
							case 4:
								dc.src_fmt = _ColFmt.BGRA;
								break;
							default:
								throw new ImageIOException("TGA: format not supported");
							}

							IFImage result = {
							w:
								dc.w, h : dc.h, c : cast(ColFmt) dc.tgt_chans,
							pixels : decode_tga(dc),};
								return result;
							}

							public void write_tga(in char[] file, long w, long h,
								in ubyte[] data, long tgt_chans = 0)
							{
								scope writer = new Writer(file);
								write_tga(writer, w, h, data, tgt_chans);
							}

							public ubyte[] write_tga_to_mem(long w, long h,
								in ubyte[] data, long tgt_chans = 0)
							{
								scope writer = new Writer();
								write_tga(writer, w, h, data, tgt_chans);
							...
							void write_tga(Writer stream, long w, long h,
								in ubyte[] data, long tgt_chans = 0)
							{
								...
								TGA_Encoder ec = {
								stream:
									stream, w : cast(ushort) w, h : cast(ushort) h,
								...
								ubyte[] decode_tga(ref TGA_Decoder dc)
								{
									...
									immutable ptrdiff_t tgt_stride = (dc.origin_at_top) ? tgt_linesize
										: -tgt_linesize;
									ptrdiff_t ti = (dc.origin_at_top) ? 0 : (dc.h - 1) * tgt_linesize;

									const LineConv convert = get_converter(dc.src_fmt,
										dc.tgt_chans);
									...
									foreach (_j; 0 .. dc.h)
									{
										...
										while (wanted)
										{
											...
											if (its_rle)
											{
												...
												for (size_t p = gotten; p < gotten + copysize;
														p += dc.bytes_pp)
													src_line[p .. p + dc.bytes_pp] = rbuf[0
														.. dc.bytes_pp];
											}
											else
											{ // it's raw
								...
								void write_tga(ref TGA_Encoder ec)
								{
									...
									{
									case 1:
										data_type = ec.rle ? Gray_RLE : Gray;
										break;
									case 2:
										data_type = ec.rle ? Gray_RLE : Gray;
										has_alpha = true;
										break;
									case 3:
										data_type = ec.rle ? TrueColor_RLE : TrueColor;
										break;
									case 4:
										data_type = ec.rle ? TrueColor_RLE : TrueColor;
										has_alpha = true;
										break;
									default:
										throw new ImageIOException("internal error");
									}
								...
								void write_image_data(ref TGA_Encoder ec)
								{
									...
									case 3:
										...
										break;
									case 4:
										tgt_fmt = _ColFmt.BGRA;
										break;
									default:
										throw new ImageIOException("internal error");
									}

									const LineConv convert = get_converter(ec.src_chans,
										tgt_fmt);
									...
									if (!ec.rle)
									{
										foreach (_; 0 .. ec.h)
										{
											convert(ec.data[si .. si + src_linesize],
												tgt_line);
									...
									foreach (_; 0 .. ec.h)
									{
										...
										ubyte[] compressed_line = rle_compress(tgt_line,
											tgt_cmp, ec.w, bytes_pp);
										ec.stream.rawWrite(compressed_line);
										si -= src_linesize; // origin at bottom
									}
								}

								ubyte[] rle_compress(in ubyte[] line,
									ubyte[] tgt_cmp, in size_t w, in int bytes_pp) pure
								{
									...
									for (size_t i = bytes_pp; pixels_left; i += bytes_pp)
									{
										...
										if (runlen < rle_limit)
										{
											...
											{ // full packet, need to store it
												...
												tgt_cmp[cmp_i .. cmp_i + copysize] = line[raw_i
													.. raw_i + copysize];
												cmp_i += copysize;
												raw_i += copysize;
												rawlen -= 128;
								...
								void read_tga_info(Reader stream, out long w,
									out long h, out long chans)
								{
									TGA_Header hdr = read_tga_header(stream);
									w = hdr.width;
									h = hdr.height;
									...
									if ((dt == TGA_DataType.TrueColor
											|| dt == TGA_DataType.Gray
											|| dt == TGA_DataType.TrueColor_RLE
											|| dt == TGA_DataType.Gray_RLE) && (hdr.bits_pp % 8) == 0)
									{
										chans = hdr.bits_pp / 8;
										return;
									}
									else if (dt == TGA_DataType.Idx || dt == TGA_DataType.Idx_RLE)
									{
										...
										case 16:
											chans = 3;
											return; // one bit could be for some "interrupt control"
										case 24:
											chans = 3;
											return;
										case 32:
								...
								public IFImage read_bmp_from_mem(in ubyte[] source,
									long req_chans = 0)
								{
									scope reader = new Reader(source);
									return read_bmp(reader, req_chans);
								...
								BMP_Header read_bmp_header(Reader stream)
								{
									...
									case 124:
										...
										break;
									default:
										throw new ImageIOException("unsupported dib version");
									}
									...
									if (1 <= dib_version)
									{
										DibV1 v1 = {
										bits_pp:
											cast(size_t) littleEndianToNative!ushort(
												dib_header[10 .. 12]),
										compression : littleEndianToNative!uint(
												dib_header[12 .. 16]),
										idat_size : cast(
												size_t) littleEndianToNative!uint(
												dib_header[16 .. 20]),
										pixels_per_meter_x : cast(
												size_t) littleEndianToNative!uint(
												dib_header[20 .. 24]),
										pixels_per_meter_y : cast(
												size_t) littleEndianToNative!uint(
												dib_header[24 .. 28]),
										palette_length : cast(
												size_t) littleEndianToNative!uint(
												dib_header[28 .. 32]),
										important_color_count : littleEndianToNative!uint(
												dib_header[32 .. 36]),};
											dib_v1 = v1;
										}
										...
										if (2 <= dib_version)
										{
											DibV2 v2 = {
											red_mask:
												littleEndianToNative!uint(dib_header[36 .. 40]),
											green_mask : littleEndianToNative!uint(
													dib_header[40 .. 44]),
											blue_mask : littleEndianToNative!uint(
													dib_header[44 .. 48]),};
												dib_v2 = v2;
											}
											...
											if (3 <= dib_version)
											{
												dib_v3_alpha_mask = littleEndianToNative!uint(
													dib_header[48 .. 52]);
											}
											...
											if (4 <= dib_version)
											{
												DibV4 v4 = {
												color_space_type:
													littleEndianToNative!uint(dib_header[52 .. 56]),
												color_space_endpoints : dib_header[56 .. 92],
												gamma_red : littleEndianToNative!uint(
														dib_header[92 .. 96]),
												gamma_green : littleEndianToNative!uint(
														dib_header[96 .. 100]),
												gamma_blue : littleEndianToNative!uint(
														dib_header[100 .. 104]),
												intent : littleEndianToNative!uint(
														dib_header[104 .. 108]),};
													dib_v4 = v4;
												}
												...
												if (5 <= dib_version)
												{
													DibV5 v5 = {
													icc_profile_data:
														littleEndianToNative!uint(
															dib_header[108 .. 112]),
													icc_profile_size : littleEndianToNative!uint(
															dib_header[112 .. 116]),};
														dib_v5 = v5;
													}
													...
													BMP_Header header = {
													file_size:
														cast(size_t) littleEndianToNative!uint(
															tmp[2 .. 6]),
													pixel_data_offset : cast(
															size_t) littleEndianToNative!uint(
															tmp[10 .. 14]),
													width : littleEndianToNative!int(
															dib_header[0 .. 4]),
													height : littleEndianToNative!int(
															dib_header[4 .. 8]),
													planes : littleEndianToNative!ushort(
															dib_header[8 .. 10]),
													dib_version : dib_version,
													dib_v1 : dib_v1,
													dib_v2 : dib_v2,
													dib_v3_alpha_mask : dib_v3_alpha_mask,
													dib_v4 : dib_v4, dib_v5 : dib_v5,};
														return header;
													}
													...
													IFImage read_bmp(Reader stream,
														long req_chans = 0)
													{
														if (req_chans < 0 || 4 < req_chans)
														...
														if (1 <= hdr.dib_version)
														{
															if (256 < hdr.dib_v1.palette_length)
																throw new ImageIOException(
																	"ivnalid palette length");
															if (hdr.dib_v1.bits_pp <= 8
																	&& (
																	hdr.dib_v1.palette_length == 0
																	|| hdr.dib_v1.compression != CMP_RGB))
																throw new ImageIOException(
																	"invalid format");
															if (
																	hdr.dib_v1.compression != CMP_RGB
																	&& hdr.dib_v1.compression != CMP_BITS)
																throw new ImageIOException(
														...
														if (rgb_masked)
														{
															...
															greeni = mask_to_idx(
																hdr.dib_v2.green_mask);
															bluei = mask_to_idx(
																hdr.dib_v2.blue_mask);
														}

														bool alpha_masked = false;
														size_t alphai = 0;
														if (3 <= hdr.dib_version
																&& hdr.dib_v3_alpha_mask != 0)
														{
															alpha_masked = true;
															alphai = mask_to_idx(
																hdr.dib_v3_alpha_mask);
														}
														...
														if (paletted)
														{
															depaletted_line = new ubyte[
																hdr.width * pe_bytes_pp];
															palette = new ubyte[
																palette_length * pe_bytes_pp];
															stream.readExact(palette[],
																palette.length);
														}

														stream.seek(hdr.pixel_data_offset,
															SEEK_SET);
														...
														const LineConv convert = get_converter(
															_ColFmt.BGRA, tgt_chans);

														immutable size_t src_linesize = hdr.width * bytes_pp; // without padding
														immutable size_t src_pad = (paletted) ? 0 : 3 - (
															(src_linesize - 1) % 4);
														immutable ptrdiff_t tgt_linesize = (
															hdr.width * cast(int) tgt_chans);

														immutable ptrdiff_t tgt_stride = (
															hdr.height < 0) ? tgt_linesize
															: -tgt_linesize;
														ptrdiff_t ti = (hdr.height < 0) ? 0 : (
															hdr.height - 1) * tgt_linesize;

														auto src_line_buf = new ubyte[src_linesize
															+ src_pad];
														auto bgra_line_buf = (paletted) ? null
															: new ubyte[hdr.width * 4];
														auto result = new ubyte[hdr.width * abs(
															hdr.height) * cast(int) tgt_chans];

														foreach (_; 0 .. abs(hdr.height))
														{
															stream.readExact(
																src_line_buf[], src_line_buf.length);
															auto src_line = src_line_buf[0
																.. src_linesize];
															...
															if (paletted)
															{
																...
																{
																	size_t i = idx * ps;
																	depaletted_line[di .. di + ps] = palette[i
																		.. i + ps];
																	if (ps == 4)
																	{
																...
																convert(depaletted_line[],
																	result[ti .. (ti + tgt_linesize)]);
															}
															else
															{
																for (size_t si, di;
																		...
																		di += 4)
																{
																	bgra_line_buf[di + 0] = src_line[si
																		+ bluei];
																	bgra_line_buf[di + 1] = src_line[si
																		+ greeni];
																	bgra_line_buf[di + 2] = src_line[si
																		+ redi];
																	bgra_line_buf[di + 3] = (
																		alpha_masked) ? src_line[si
																		+ alphai] : 255;
																}
																convert(bgra_line_buf[],
																	result[ti .. (ti + tgt_linesize)]);
															}
															...
															out long chans)
														{
															...
															chans = (
																hdr.dib_version >= 3
																&& hdr.dib_v3_alpha_mask != 0) ? ColFmt.RGBA
																: ColFmt.RGB;
														}
														...
														public IFImage read_jpeg(
															in char[] filename, long req_chans = 0)
														{
															scope reader = new Reader(filename);
															return read_jpeg(reader,
																req_chans);
														}

														public IFImage read_jpeg_from_mem(
															in ubyte[] source, long req_chans = 0)
														{
															scope reader = new Reader(source);
															return read_jpeg(reader,
																req_chans);
														...
														IFImage read_jpeg(
															Reader stream, long req_chans = 0)
														{
															if (req_chans < 0 || 4 < req_chans)
																throw new ImageIOException(
																	"come on...");
															...
															stream:
																...
																dc.tgt_chans = (req_chans == 0) ? dc.num_comps
																	: cast(int) req_chans;
																...
																struct JPEG_Decoder
																{
																	...
																	struct Component
																	{
																		ubyte id;
																		ubyte sfx,
																			sfy; // sampling factors, aka. h and v
																...
																void read_markers(ref JPEG_Decoder dc)
																{
																	...
																	while (
																			!has_next_scan_header
																			&& !dc.eoi_reached)
																	{
																		ubyte[2] marker;
																		dc.stream.readExact(marker,
																			2);
																		...
																		debug (DebugJPEG)
																			writefln("marker: %s (%1$x)\t",
																				cast(Marker) marker[
																				1]);
																		switch (marker[1]) with (
																				Marker)
																		{
																		case DHT:
																			dc.read_huffman_tables();
																			break;
																		case DQT:
																			dc.read_quantization_tables();
																			break;
																		case SOF0:
																		...
																		case COM:
																			debug (DebugJPEG)
																				writefln(
																					"-> skipping segment");
																			ubyte[2] lenbuf = void;
																			dc.stream.readExact(lenbuf,
																				lenbuf.length);
																			int len = bigEndianToNative!ushort(
																				lenbuf) - 2;
																			dc.stream.seek(len,
																				SEEK_CUR);
																			break;
																...
																void derive_mincode_maxcode_valptr(
																	ref short[16] mincode,
																	ref short[16] maxcode,
																	ref short[16] valptr,
																	in ref short[256] codes,
																	in ref ubyte[16] num_values) pure
																{
																	mincode[] = -1;
																	maxcode[] = -1;
																	valptr[] = -1;
																	...
																	ref JPEG_Decoder dc)
																{
																	...
																	while (0 < len)
																	{
																		...
																		if (
																				3 < table_slot
																				|| precision != 0) // only 8 bit for baseline
																			throw new ImageIOException(
																				"invalid / not supported");

																		dc.stream.readExact(
																			dc.qtables[table_slot],
																			64);
																		len -= 1 + 64;
																	}
																}
																	...
																	ref JPEG_Decoder dc)
																{
																	...
																	dc.stream.readExact(tmp,
																		8);
																	int len = bigEndianToNative!ushort(
																		tmp[0 .. 2]); // 8 + num_comps*3
																	ubyte precision = tmp[2];
																	dc.height = bigEndianToNative!ushort(
																		tmp[3 .. 5]);
																	dc.width = bigEndianToNative!ushort(
																		tmp[5 .. 7]);
																	dc.num_comps = tmp[7];
																	...
																	foreach (i; 0 .. dc.num_comps)
																	{
																		...
																		comp.id = ci;
																		ubyte sampling_factors = tmp[i * 3
																			+ 1];
																	...
																	foreach (i; 0 .. dc.num_comps)
																	{
																		dc.comps[i].x = cast(long) ceil(
																			dc.width * (
																			cast(double) dc.comps[i].sfx / dc.hmax));
																		dc.comps[i].y = cast(long) ceil(
																			dc.height * (
																			cast(double) dc.comps[i].sfy / dc.vmax));

																		debug (DebugJPEG)
																			writefln("%d comp %d sfx/sfy: %d/%d",
																				i,
																				dc.comps[i].id,
																				dc.comps[i].sfx,
																				dc.comps[i].sfy);
																	...
																	dc.num_mcu_x = cast(int)(
																		(dc.width + mcu_w - 1) / mcu_w);
																	dc.num_mcu_y = cast(int)(
																		(dc.height + mcu_h - 1) / mcu_h);
																	...
																	ref JPEG_Decoder dc)
																{
																	...
																	dc.stream.readExact(tmp,
																		tmp.length);
																	ushort len = bigEndianToNative!ushort(
																		tmp[0 .. 2]);
																	...
																	ubyte[] buf;
																	buf.length = len - 3;
																	dc.stream.readExact(buf,
																		buf.length);

																	foreach (i; 0 .. num_scan_comps)
																	{
																		uint comp_id = buf[i * 2] - (
																			(dc.correct_comp_ids) ? 1
																			: 0);
																		int ci; // component index
																		while (
																				ci < dc.num_comps
																				&& dc.comps[ci].id != comp_id)
																			++ci;
																		if (dc.num_comps <= ci)
																			throw new ImageIOException(
																				"invalid component id");
																	...
																	ref JPEG_Decoder dc)
																{
																	...
																	if (len != 4)
																		throw new ImageIOException(
																			"invalid / not supported");
																	dc.restart_interval = bigEndianToNative!ushort(
																		tmp[2 .. 4]);
																	debug (DebugJPEG)
																		writeln(
																	...
																	ref JPEG_Decoder dc)
																{
																	foreach (ref comp;
																			...
																			.. dc.num_comps])
																		comp.data = new ubyte[
																			dc.num_mcu_x * comp.sfx * 8 * dc.num_mcu_y * comp.sfy * 8];
																...
																void decode_scan(ref JPEG_Decoder dc)
																{
																	...
																	if (0 < dc.restart_interval)
																	{
																		...
																		intervals = (
																			total_mcus + dc.restart_interval - 1) / dc.restart_interval;
																			...
																			0 .. dc.num_mcu_y)
																	{
																				...
																				0 .. dc.num_mcu_x)
																		{
																					...
																					0 .. dc.num_comps)
																			{
																						...
																						0 .. comp.sfy)
																				{
																							...
																							0 .. comp.sfx)
																					{
																						...
																						// idct & level-shift
																						long outx = (
																							mcu_i * comp.sfx + du_i) * 8;
																						long outy = (
																							mcu_j * comp.sfy + du_j) * 8;
																						long dst_stride = dc.num_mcu_x * comp
																							.sfx * 8;
																						ubyte* dst = comp.data.ptr + outy * dst_stride + outx;
																						stbi__idct_block(dst,
																							dst_stride,
																							data);
																					}
																				}
																			...
																			if (!mcus)
																			{
																				...
																				read_restart(
																					dc.stream); // RSTx marker

																				if (intervals == 1)
																				{
																					// last interval, may have fewer MCUs than defined by DRI
																					mcus = (
																						dc.num_mcu_y - mcu_j - 1) * dc.num_mcu_x + dc.num_mcu_x - mcu_i - 1;
																				}
																				else
																				{
																					mcus = dc.restart_interval;
																...
																short[64] decode_block(
																	ref JPEG_Decoder dc,
																	ref JPEG_Decoder.Component comp,
																	in ref ubyte[64] qtable)
																{
																	short[64] res = 0;

																	ubyte t = decode_huff(dc,
																		dc.dc_tables[comp.dc_table]);
																	int diff = t ? dc.receive_and_extend(
																		t) : 0;
																	...
																	in ref HuffTab tab)
																{
																	...
																	while (tab.maxcode[i] < code)
																	{
																		...
																		if (tab.maxcode.length <= i)
																			throw new ImageIOException(
																				"corrupt huffman coding");
																	}
																	int j = tab.valptr[i] + code - tab.mincode[
																		i];
																	if (
																			tab.values.length <= cast(
																			uint) j)
																		throw new ImageIOException(
																			"corrupt huffman coding");
																	return tab.values[j];
																...
																ubyte[] reconstruct(
																	in ref JPEG_Decoder dc)
																{
																	auto result = new ubyte[
																		dc.width * dc.height * dc.tgt_chans];

																	switch (
																			dc.num_comps * 10 + dc.tgt_chans)
																	{
																	case 34,
																			33:
																			foreach (const ref comp;
																					dc.comps[0
																					.. dc.num_comps])
																			{
																				if (
																...
																ubyte[] upsample_gray(
																	...
																	ubyte[] result)
																{
																	const size_t stride0 = dc.num_mcu_x * dc.comps[
																		0].sfx * 8;
																	const double si0yratio = cast(
																		double) dc.comps[0].y / dc.height;
																	const double si0xratio = cast(
																		double) dc.comps[0].x / dc.width;
																	size_t si0, di;

																	foreach (j; 0 .. dc.height)
																	{
																		si0 = cast(size_t) floor(
																			j * si0yratio) * stride0;
																		foreach (i;
																				0 .. dc.width)
																		{
																			result[di] = dc.comps[0].data[si0 + cast(
																				size_t) floor(
																				i * si0xratio)];
																			if (dc.tgt_chans == 2)
																				result[di + 1] = 255;
																	...
																	ubyte[] result)
																{
																	const size_t stride0 = dc.num_mcu_x * dc.comps[
																		0].sfx * 8;
																	const size_t stride1 = dc.num_mcu_x * dc.comps[
																		1].sfx * 8;
																	const size_t stride2 = dc.num_mcu_x * dc.comps[
																		2].sfx * 8;

																	const double si0yratio = cast(
																		double) dc.comps[0].y / dc.height;
																	const double si1yratio = cast(
																		double) dc.comps[1].y / dc.height;
																	const double si2yratio = cast(
																		double) dc.comps[2].y / dc.height;
																	const double si0xratio = cast(
																		double) dc.comps[0].x / dc.width;
																	const double si1xratio = cast(
																		double) dc.comps[1].x / dc.width;
																	const double si2xratio = cast(
																		double) dc.comps[2].x / dc.width;
																	size_t si0, si1,
																		si2, di;
																	...
																	foreach (j; 0 .. dc.height)
																	{
																		si0 = cast(size_t) floor(
																			j * si0yratio) * stride0;
																		si1 = cast(size_t) floor(
																			j * si1yratio) * stride1;
																		si2 = cast(size_t) floor(
																			j * si2yratio) * stride2;
																		...
																		{
																			result[di .. di + 3] = ycbcr_to_rgb(
																				dc.comps[0].data[si0 + cast(
																				size_t) floor(
																				i * si0xratio)],
																				dc.comps[1].data[si1 + cast(
																				size_t) floor(
																				i * si1xratio)],
																				dc.comps[2].data[si2 + cast(
																				size_t) floor(
																				i * si2xratio)],
																				);
																			if (dc.tgt_chans == 4)
																	...
																	out long h, out long chans)
																{
																	...
																	while (true)
																	{
																		...
																		case SOF9: .. case SOF11:
																			ubyte[8] tmp;
																			stream.readExact(tmp[0 .. 8],
																				8);
																			//int len = bigEndianToNative!ushort(tmp[0..2]);
																			w = bigEndianToNative!ushort(
																				tmp[5 .. 7]);
																			h = bigEndianToNative!ushort(
																				tmp[3 .. 5]);
																			chans = tmp[7];
																			return;
																		case SOS,
																		...
																		case SKIP:
																			ubyte[2] lenbuf = void;
																			stream.readExact(lenbuf,
																				2);
																			int skiplen = bigEndianToNative!ushort(
																				lenbuf) - 2;
																			stream.seek(skiplen,
																				SEEK_CUR);
																			break;
																...
																alias LineConv = void function(
																	in ubyte[] src,
																	ubyte[] tgt);

																LineConv get_converter(
																	long src_chans,
																	long tgt_chans) pure
																{
																	long combo(long a,
																...
																ubyte luminance(
																	ubyte r, ubyte g,
																	ubyte b) pure nothrow
																{
																	return cast(ubyte)(
																		0.21 * r + 0.64 * g + 0.15 * b); // somewhat arbitrary weights
																}
																...
																private:
																	...
																	{
																		if (
																				source.length - cursor < bytes)
																			throw new Exception(
																				"not enough data");
																		buffer[0 .. bytes] = source[cursor
																			.. cursor + bytes];
																		cursor += bytes;
																	}
																...
																const(char)[] extract_extension_lowercase(
																	in char[] filename)
																{
																	ptrdiff_t di = filename.lastIndexOf(
																		'.');
																	return (0 < di
																		&& di + 1 < filename.length) ? filename[di
																		+ 1 .. $].toLower() : "";
																}
