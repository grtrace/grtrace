...
void drawPolygon(const(float)* coords, uint numCoords, float r, uint col)
{
	...
	const float[4] colf = [
		...
		cast(float)((col >> 16) & 0xff) / 255.0, cast(float)((col >> 24) & 0xff) / 255.0
	];
...
{
	const float[4 * 2] verts = [x + 0.5f, y + 0.5f, x + w - 0.5f, y + 0.5f,
		x + w - 0.5f, y + h - 0.5f, x + 0.5f, y + h - 0.5f,];
	...
	foreach (dchar c; cast(string) text)
	{
		...
		else if (c >= FIRST_CHARACTER && c < FIRST_CHARACTER + g_max_character_count)
		{
			...
			float[12] v = [q.x0, q.y0, q.x1, q.y1, q.x1, q.y0, q.x0, q.y0,
				q.x0, q.y1, q.x1, q.y1,];
			float[12] uv = [q.s0, q.t0, q.s1, q.t1, q.s1, q.t0, q.s0, q.t0,
				q.s0, q.t1, q.s1, q.t1,];
			float[24] cArr = [r, g, b, a, r, g, b, a, r, g, b, a, r, g, b, a,
				r, g, b, a, r, g, b, a,];
			glBindVertexArray(g_vao);
			glBindBuffer(GL_ARRAY_BUFFER, g_vbos[0]);
