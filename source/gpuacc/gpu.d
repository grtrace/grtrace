...
import config;
import std.math, std.algorithm, std.string, std.format, std.conv, std.range,
	std.array, std.stdio;
...
void checkCuda(CUresult err, string file = __FILE__, int line = __LINE__)
{
	if (err != CUDA_SUCCESS)
	{
		...
		throw new CudaException("Cuda error #%.04d [%s] in file %s @ %d".format(err,
			xstr, file, line));
	}
}
		...
		for (int i = 0; i < _arguments.length; i++)
		{
			...
			else
			{
				throw new CudaException(
					"Unsupported argument type for CUDA kernel call: " ~ _arguments[i].toString());
			}
		}
		checkCuda(cuLaunchKernel(gfunc, gsz.x, gsz.y, gsz.z, tsz.x, tsz.y,
			tsz.z, sharedBytes, null, kargs.ptr, null));
	}
}
...
void initCuda()
{
	...
	checkCuda(cuDeviceGetAttribute(&major, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR,
		device));
	checkCuda(cuDeviceGetAttribute(&minor, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR,
		device));
...
public void InitGPU()
{
	try
	{
		...
		static if (false) // Sample
		{
			...
			vadd.Call(GPUGridSz(1, 1, 1), GPUThreadSz(A.length, 1, 1), 32,
				&gA, &gB, &gC, cast(int)(A.length));
			SynchronizeGPU();
