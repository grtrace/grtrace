...
class Radial : CoordinateChanger
{
	...
	fpnum[4] transformForwardPosition(Vectorf a)
	{
		...
		fpnum rt = ~tmp; fpnum tt = acos(tmp.z/rt); fpnum pt = atan2(tmp.y,tmp.x);
		return [0, rt, tt, pt];
	}
	...
	fpnum[4] transformForwardSpacialFirstDerivatives(Vectorf pos, Vectorf dir)
	{
		...
		return [
			0,
			(tmp*dir)/r,
			((tmp.z/w)*(tmp.x*dir.x + tmp.y*dir.y) - w*dir.z)/r2,
	...
	Vectorf transformBackSpacialFirstDerivatives(fpnum[4] pos, fpnum[4] d1)
	{
		...
		return Vectorf(
			sinT*cosP*d1[1] + pos[1]*(cosT*cosP*d1[2] - sinT*sinP*d1[3]),
	...
	Vectorf transformBackSpacialSecondDerivatives(fpnum[4] pos, fpnum[4] d1, fpnum[4] d2)
	{
		...
		return Vectorf(
			sinT*cosP*d2[1] + 2*d1[1]*(cosT*cosP*d1[2]-sinT*sinP*d1[3]) + pos[1]*(cosT*(cosP*d2[2]-2*sinP*d1[2]*d1[3]) - sinT*(cosP*(d1[2]*d1[2]+d1[3]*d1[3])+sinP*d2[3])),
			sinT*sinP*d2[1] + 2*d1[1]*(cosT*sinP*d1[2]+sinT*cosP*d1[3]) + pos[1]*(sinT*(cosP*d2[3]-sinP*(d1[2]*d1[2]+d1[3]*d1[3])) + cosT*(sinP*d2[2]+2*cosP*d1[2]*d1[3])),
