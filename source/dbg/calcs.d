...
__gshared AdditionalCalculation[2] calculations = [
...
void calcTotalDistantRayDeflection()
{
	...
	if (rayTracingDistance <= 0)
		max_num_of_steps = size_t.max;
	else
		max_num_of_steps = cast(int)(rayTracingDistance / timestep);
	...
	f.writef(format(
			...
			timestep, distFromHole,
			castingDistFromHoleMin, castingDistFromHoleMax, castingDistFromHoleStep)); //simulation info
	...
	int stepi = 0;
	int numsteps = cast(int)(
			(castingDistFromHoleMax - castingDistFromHoleMin) / castingDistFromHoleStep);
			...
			curCastingDist -= castingDistFromHoleStep)
	{
		...
		Line ray = Line(vectorf(0, distFromHole, 0), vectorf(curCastingDist,
				-distFromHole, 0).normalized);
		ray.ray = true;
		...
		f.writef("%#.16e\t%#.16e\t%#.16e\t%#.16e\n", minDistToHole,
				cOfMotion["E"], cOfMotion["L"], totalDeflection);
		f.flush();
