...
void StartTest()
{
	...
	if(ans=='y')
	{
		...
		if(inp == 1)
		{
			...
			double mass, min_timestep, max_timestep, step_timestep;
			int numOfCircles;
			string path;
			do
			{
				printf("Enter: mass min_timestep max_timestep step_timestep maxNumOfCircles:\n");
				scanf("%lf %lf %lf %lf %d", &mass, &min_timestep, &max_timestep, &step_timestep, &numOfCircles);
						...
						step_timestep<=0. || numOfCircles<0)?(printf("Try Again\n"),1):0)); //RIP syntax
			printf("Enter Directory to Save Results:\n");
...
void PhotonSphereStability(fpnum mass, fpnum min_timestep, fpnum max_timestep, fpnum step_timestep, int numOfCircles, string dir)
{
	...
	VisualDebugger.inst.rays = [];
	VisualDebugger.inst.rayProcesorData = [9*mass*mass, 0.0, 0.0, 0.0];
	...
	for(; timestep<max_timestep; i++)
	{
		timestep = min_timestep + i*step_timestep;
		//create file
		auto writer = appender!string();
		formattedWrite(writer, "T:-%#.4f-M-%#.4f.data", timestep, mass);
		VisualDebugger.inst.rayProcesorFile = File(dir~"/"~writer.data, "w");
		writeln("Writing to: "~dir~"/"~writer.data);
		
		//file header
		writer = appender!string();
		formattedWrite(writer, "Mass: %#.16e \t Timestep: %#.16e \t CirclesMax: %d \n\n", mass, timestep, numOfCircles);
		VisualDebugger.inst.rayProcesorFile.write("File_Version: 1.0 \t Generated by GrTrace \n");
		VisualDebugger.inst.rayProcesorFile.write(writer.data);
		VisualDebugger.inst.rayProcesorFile.write("EPS\n");
		...
		//close file
		VisualDebugger.inst.rayProcesorFile.flush();
		VisualDebugger.inst.rayProcesorFile.close();
	}
...
class VisualDebugger
{
	...
	fpnum[] rayProcesorData;
	auto rayProcesorWriter = appender!string();
	File rayProcesorFile;
	...
	static public void PhotonSphereStabilityRayProcesor(Line ray, Vectorf newp, Color* col = null)
	{
		...
		fpnum a = half.x-inst.rayProcesorData[1];
		fpnum b = half.y-inst.rayProcesorData[2];
		fpnum c = half.z-inst.rayProcesorData[3];
		fpnum rad = a*a+b*b+c*c;
		fpnum eps = rad/inst.rayProcesorData[0];
		
		inst.rayProcesorWriter = appender!string();
		formattedWrite(inst.rayProcesorWriter, "%#.16e\n", eps);
		inst.rayProcesorFile.write(inst.rayProcesorWriter.data);
	}
